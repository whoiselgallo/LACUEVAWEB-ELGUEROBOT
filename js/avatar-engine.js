...
function generarPropObjeto() {
    ...
    if (output) {
        // Prompt vectorial y limpio para props del escenario
        const promptProp = `Premium flat vector asset of ${objeto}, detailed game asset, bold outlines, clean SVG path, flat 2d design, neon cyber-punk style, glowing accent, isolated on solid white transparent background, no shadows, no text, no frame, sticker graphic design`;
        const encoded = encodeURIComponent(promptProp);
        const imageUrl = `https://image.pollinations.ai/prompt/${encoded}?width=600&height=600&nologo=true&private=true&enhance=true`;
        
        output.innerHTML = `
            <div style="background:rgba(255,0,255,0.05); border:1px solid var(--neon-magenta); border-radius:12px; padding:20px; text-align:center;">
                <h4 style="color:#FF00FF; margin:0 0 15px; text-align:left;"><i class="fa-solid fa-couch"></i> Objeto / Prop Generado (Estilo Vector Neón)</h4>
                
                <!-- RENDERIZAR LA IMAGEN DEL OBJETO REAL -->
                <div style="background:radial-gradient(circle, rgba(20,20,30,0.9), rgba(5,5,10,0.95)); border:1px solid rgba(255,0,255,0.15); border-radius:10px; padding:15px; display:inline-block; margin-bottom:15px; box-shadow: 0 0 25px rgba(255,0,255,0.15);">
                    <img src="${imageUrl}" style="max-height:180px; border-radius:8px; display:block; filter: drop-shadow(0 0 12px rgba(255,0,255,0.4));" alt="${objeto}">
                </div>

                <p style="margin:0 0 15px; font-size:0.9rem; color:#fff; text-align:left;">Objeto: <strong>${objeto}</strong> | Formato Vectorial Flat</p>

                <!-- ACCIONES DE INTEGRACIÓN DIRECTA -->
                <div style="display:flex; gap:10px; justify-content:center; margin-bottom:10px;">
                    <button class="btn-neon" onclick="cargarImagenDesdeUrl('${imageUrl}')"><i class="fa-solid fa-palette"></i> Cargar en Canva</button>
                    <a href="${imageUrl}" download="prop_${objeto.replace(/\s+/g, '_')}.png" class="btn-neon btn-neon-magenta" style="text-decoration:none; display:inline-block; padding:10px 20px;"><i class="fa-solid fa-download"></i> Descargar PNG</a>
                </div>
            </div>
        `;
    }
