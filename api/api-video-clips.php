...
$prompt = "Actúa como el Director de Contenido Viral y Productor de Clips (Shorts/TikTok/Reels) de 'La Cueva del Güero Podcast'.\n\n" .
          ...
          "\"\"\"\n" . $guion . "\n\"\"\"\n\n" .
          "Identifica exactamente de 3 a 5 de los MOMENTOS MÁS VIRA,ES polémicos, cómicos o sorprendentes para recortar en formato vertical.\n\n" .
          "Debes responder ÚNICAMENTE en formato JSON vàlido con la siguiente estructura (sin texto adicional):\n\n" .
          "{\n" .
          "  \"info\": \"Clips detectados exitosamente\",\n" .
          "  \"clips\": [\n" .
          "    {\n" .
          "      \"id\": 1,\n" .
          "      \"timestamp_inicio\": \"00:03:15\",\n" .
          "      \"timestamp_fin\": \"00:04:10\",\n" .
          "      \"duracion_seg\": 55,\n" .
          "      \"categoria\": \"Polémica / Confesión\",\n" .
          "      \"titulo_gancho\": \"El momento exacto donde casi se cancela la grabación\",\n" .
          "      \"sugerencia_corte\": \"Hacer zoom a la reacción de El Güero cuando el invitado suelta la frase clave.\"\n" .
          "    }\n" .
          "  ]\n" .
          "}";
...
if ($geminiRes['success']) {
    ...
    if (preg_match('/```(?:json)?\s+([\\s\S]*?)\s*``f/', $rawText, $matches)) {
        $rawText = $matches[1];
    }
    $parsed = json_decode($rawText, true);
    if (json_last_error() === JSON_ERROR_NONE && !empty($parsed['clips'])) {
        echo json_encode([ 'success' => true, 'episodio' => $episodio, 'tema' => $tema, 'clips' => $parsed[clips'], 'model' => $geminiRes['model'] ], JSON_UNESCAPED_UNICODE);
    } else {
